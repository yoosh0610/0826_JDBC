...
public class MemberController {
	/**
	 * VIEW로 부터 전달받은 문자열값 네개를 DAO로 전달하는 메소드
	 * 
	 ...
	 * 
	 * @return : 반환 어떻게 할건지
	 * @throws : IllgalArgumenException : 잘못된 인자가 전달되면 발생할 수 있음
	 */
	public int save(String userId, String userPwd, String userName, String email) {
		// 1. 데이터 가공 => 넘어온 인자값이 두개 이상일 시
		//				   어딘가에 담을 것(VO, Map, DTO)
		// 테이블에 한 행에 담기위해 만들었으니 Member의 필드에 담는다.
		// 전달된 인자값들을 Member객체의 필드에 담기!
		// 1) 매개변수 생성자를 호출하여 객체 생성과 동시에 필드값을 개입하는 방법
	 ...
	 * 
	 * @param userId : 사용자가 입력한 검색하고다 하는 문자열
	 */
	...
	public List<Member> findByKeyword(String keyword) {
		// 결과값이 나중에 어떻게 돌아올까?
		// SELCET -> ResultSet -> Member -> List<Member>
	...
	public int update(String userId, String userPwd, String newPassword) {
		...
		// 1. 데이터 가공(인자값이 여러개일 때 어디에 담을지 작업)
		// DTO에 새로운 값들을 담아주자!!
		PasswordDTO pd = new PasswordDTO(userId, userPwd, newPassword);
		
		// 2. 요청처리
		// 아이디랑 비밀번호랑 바꿀 비밀번호 줄테니까
		// 이거 아이디랑 비밀번호 맞는지 확인하고 바꿀 비밀번호로 비밀번호컬럼값 바꿔줘
		// DAO야 이거 줄게 해줘
