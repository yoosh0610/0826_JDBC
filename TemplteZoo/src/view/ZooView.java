...
import model.controller.ZooController;
import model.vo.Zoo;
...
public class ZooView {
	...
	private void save() {
		...
		int result = zc.save(new Zoo(animalName, animalType, zone, prey, gender));
		if(result > 0) {
...
private void update() {
		...
		System.out.print("동물이름을 입력해주세요. > ");
		String animalName = sc.nextLine();
		System.out.print("동물종류를 입력해주세요. > ");
		String animalType = sc.nextLine();
		System.out.print("새 동물종류를 입력해주세요. > ");
		String newType = sc.nextLine();
		
		int result = zc.update(animalName, animalType, newType);
		
		if(result > 0) {
			System.out.println("동물 변경에 성공했습니다.");
		} else {
			System.out.println("동물 변경에 실패했습니다.");
		}
	}
	
private void delete() {
	System.out.println("동물 삭제 ");
	System.out.print("동물명 입력하세요. > ");
	String animalName = sc.nextLine();
	System.out.print("동물종류 입력하세요. > ");
	String animalType = sc.nextLine();
	
	int result = zc.delete(animalName, animalType);
