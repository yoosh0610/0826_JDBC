...
public class PlantView {
	...
	private void update() {
		...
		String plType = sc.nextLine();
		System.out.print("새 식물 종류를 입력해주세요. > ");
		String newType= sc.nextLine();
		
		int result = pc.update(plName, plType, newType);
		
		if(result > 0) {
			System.out.println("종류 변경에 성공했습니다.");
		} else {
			System.out.println("종류 변경에 실패했습니다.");
		}
	}
	
	private void delete() {
		System.out.println("식물 삭제");
		System.out.print("식물명 입력하세요. > ");
		String plName = sc.nextLine();
		System.out.print("식물종류 입력하세요. > ");
		String plType = sc.nextLine();
		
		int result = pc.delete(plName, plType);
